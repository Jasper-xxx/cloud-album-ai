<template>
    <div id="picture" class="content">
                ...
                <div class="album-header-info-name">
                    ...
                    <el-breadcrumb :separator-icon="ArrowRight" class="album-header-breadcrumb">
                        <el-breadcrumb-item @click="router.back()"
                            class="album-breadcrumb-item1">设备</el-breadcrumb-item>
                        <el-breadcrumb-item class="album-breadcrumb-item2">{{ makeName == ' ' ? '其它' : makeName + "-" +
                            modelName
                            }}</el-breadcrumb-item>
                    </el-breadcrumb>
        ...
        <div class="pictureList">
            <!-- 图片列表 -->
            <PictureList parentComponent="ModelAlbum" :fileInfoList="fileInfoList" :moreFileInfoList="moreFileInfoList"
...
import { selectModelAlbumFileInfo } from '@/api/album/album'
...
const modelName = ref(String(route.params.modelName));
const makeName = ref(String(route.params.makeName));
...
const handleSelectAllFileInfo = () => {
    ...
    selectModelAlbumFileInfo(pageRequest.value).then(res => {
        if (res.code === 200) {
...
const loadMoreData = async () => {
    ...
    pageRequest.value.current += 1
    try {
        const res = await selectModelAlbumFileInfo(pageRequest.value)
        if (res.code === 200) {
